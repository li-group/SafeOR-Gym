...
class Generator_transmission_expansion_env(gym.Env):
    ...
    State space:
        The state space is a dictionary with the following keys:
            ...
            - num_trans: A dictionary with keys as transmission lines and the value being 1 if the transmission line is installed and 0 if it is not installed.
    ...
    Transition to next state:
        The num_gen is updated by adding the corresponding addgen value after adjusting for bound constraints. The num_trans for a transmission line is updated to 1 
        if there is power flow in the transmission line and the num_trans was 0 for the previous time period.
    Cost:
    ...
    def __init__(self, env_id: str,
        ...
        self.max_gen = 100
        self.max_transline = 1
        self.max_demand_value = max(max(region_demand.values()) for region_demand in self.demand.values())
        self.obs_high = np.concatenate([np.repeat(self.max_gen,len(self.generators)*len(self.regions)),np.repeat(self.max_transline,len(self.transmission_lines)),np.repeat(self.max_demand_value,len(self.regions)),np.repeat(self.T,1)])
        ...
        for l in self.transmission_lines:  
            low_list.append(-self.transcap[l])
            high_list.append(self.transcap[l])
    ...
    def get_new_start_state(self):
        ...
        print(self.demand)
        self.state = {"num_gen" : {(i,r):0 for i in self.generators for r in self.regions}, "num_trans":{l:0 for l in self.transmission_lines},
    ...
    def check_bounds_cost(self,action):
        #Function to check the bounds of the action. If the action is out of bounds, set it to the bounds and calculte the penalty for the violation to the cost.
        #The penalty is the sum of a scaled L0 violation and a scaled L2 violation for the add gen part and a scaled L0 violation for the bounds of the powerflow in
        # transmission line. 
    ...
    def update_num_gen_tl(self,action):
    ...
    def compute_genreward(self,action):
    ...
    def compute_tlreward(self,action):
    #Function to calculate the reward for the installation of the transmission lines. 
    #It is calulcated the negative of the installation cost of the transmission lines added.
        tl_reward = 0
        if(len(self.transmission_lines)>0):
            for l in self.transmission_lines:
                if(action["powflow"][l]!=0 and self.state["num_trans"][l]==0):
    ...
    def check_dem_cost(self,action):
        #Function to check the violations of the demand constraints. If the demand is not satisfied, calculate a penalty for the violation.
        ...
        print("action tuple",action["powflow_tup"])
        for r in self.regions:
            ...
            if(self.state["Dem"][r]> total_avail):
                ...
                self.pens_step['Total penalty: Demand violations']+=self.D*(self.state["Dem"][r]-total_avail)**2+self.P
        return dem_cost

    def sanitize_action(self,action):
        #Function to santize the action. In the case of generators, we round the action to the nearest integer. In the case of transmission lines we round neglible 
        # power flows to 0.
        if(len(self.transmission_lines)>0):
    ...
    def step(self, action_scaled: th.Tensor):
        ...
        print("mapping",self.mapping_act)
        ...
        reward = self.compute_genreward(action)+self.compute_tlreward(action)
        self.update_num_gen_tl(action)
        cost = bounds_cost+self.check_dem_cost(action)
