...
def cfg_to_omni(env_config,cfg_id: int, layout, device = "cuda:0", total_steps = 40, parallel=1, algo = "CPO"):
...
class Cap_exp_env_Omni(CMDP):
    ...
    def step(self, action_unscaled: th.Tensor):
        """
        ...
        """
        low_torch = torch.tensor(self.action_low, dtype=action_unscaled.dtype, device=self._device)
        high_torch = torch.tensor(self.action_high, dtype=action_unscaled.dtype, device=self._device)
        action = (action_unscaled + 1) / 2 * (high_torch - low_torch) + low_torch
    ...
    def get_new_start_state(self):
        self.state = {"num_gen" : {(i,r):0 for i in self.generators for r in self.regions},
                      "num_trans":{l:0 for l in self.transmission_lines},"Dem":{r:self.demand[r][str(1)] for r in self.regions}
                      }
        self.state["t"] = self.t
...
print(reward)
print(env.state)
action_1 = th.tensor([0,0,5]).to("cuda:0")
state, reward,cost, done,truncated, info = env.step(action_1)
env.render()
print(reward)
env.close()'''
...
if __name__ == "__main__":
    ...
    'transdict': {"1": "r1_r2"}}
    cfg = cfg_to_omni(env_config,cfg_id=77, layout=layout, algo=ALGO)
    #print(cfg)
