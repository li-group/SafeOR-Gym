...
class STNEnv(gym.Env):
    ...
    Env Registration : stn-v0

    The State Task Network (STN) is a modeling framework for plant scheduling which
    ...
    The following functions are included:
        ...
        a cost to the agent in that case
        ...
        - _scheduler_products : computes the delivery of products at that time step due to previous action (task) execution
    ...
    def __init__(self, env_id: str, **kwargs: Any) -> None:
        ...
        self.tasks = []
        self.min_batch = np.zeros((self.num_tasks, len(self.equipments)))
        self.max_batch = np.zeros((self.num_tasks, len(self.equipments)))
        min_batch = []
        max_batch = []
        self.task_utilities = []
        ...
        self.task_taus = [self.tasks_dict[name].get('p') for name in self.task_names]
        self.max_tau = max([v for t in self.task_taus for k, v in t.items()])
        self.max_equipment = max([len(v['equipments']) for k, v in self.tasks_dict.items()])
        ...
        for task_name in self.task_names:
            ...
            # Ki (the units that can perform the task)
            task_equipments = []
            # The processing time to produce resource r from task i
            #task_pr = task_data['p']
            # The min batch size for task i when using equipment j
            vmin = task_data['Vmin']
            # The min batch size for task i when using equipment j
            vmax = task_data['Vmax']

            min_batch.append(vmin)
            max_batch.append(vmax)
        ...
        for i, t in enumerate(min_batch):
            for e, b in t.items():
                self.min_batch[i, self.equipments.index(e)] = b

        for i, t in enumerate(max_batch):
            for e, b in t.items():
                self.max_batch[i, self.equipments.index(e)] = b
    ...
    def _compute_resource_change(self, action: np.ndarray) -> np.ndarray:
        ...
        for i, stoich in enumerate(self.task_stoichs):
            ...
            total_change += np.sum(action[i], axis = -1) * change
    ...
    def sanitize_action(self, raw_action: np.ndarray) -> np.ndarray:
        ...
        Sanitize and scale raw actions from the RL policy output.
            - raw_action is assumed to be in [-1, 1]^num_tasks x max_equipments.
        ...
        for i in range(self.num_tasks):
            for e, equip in enumerate(self.equipments):
                scaled_batch = scaled_action[i, e]
                ...
                if max_feasible_batch < scaled_batch:
                    ...
                    if max_feasible_batch < self.min_batch[i, e]:
                        candi_action[i, e] = 0.0  # Infeasible action, set to 0
                    else:
                        candi_action[i, e] = max_feasible_batch  # Clamp batch to feasible amount

                    self.logger.debug(f"---- Task {i+1} on equipment {e+1} clamped to {candi_action[i, e]:.3f} ----")
                    self.env_spec_log["Penalties/inv_lb"] += 1
                else:
                    # Action is feasible, keep the scaled batch as predicted by the agent
                    candi_action[i, e] = scaled_batch
        ...
        for i in range(self.num_tasks):
            for e, equip in enumerate(self.equipments):
                ...
                if duplicate_inventory[self.resources.index(equip)] <= 0:
                    ...
                    final_action[i, e] = candi_action[i, e]
                    # reduce no of units of that equipment
                    duplicate_inventory[self.resources.index(equip)] -= 1
    ...
    def step(self, action) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        if not feasible:
            ...
            for i in range(self.num_tasks):
                for e, equip in enumerate(self.equipments):
                    tau = self.task_taus[i]
                    last_tau = max([t for _, t in tau.items()])
                    
                    if not sanitized_action[i, e]:
                        continue
                    # For every equipment required, consume one unit immediately.
                
                    new_inventory[self.resources.index(equip)] -= 1
                    ret_time = self.t + last_tau

                    self.delayed_production_queue.setdefault(ret_time, [])
                    self.delayed_production_queue[ret_time].append((equip, 1.0))
                    ...
                    # if self.t + last_tau not in self.delayed_production_queue['equipment_returns']:
                    #     self.delayed_production_queue['equipment_returns'][self.t + last_tau] = []
                    # self.delayed_production_queue['equipment_returns'][self.t + last_tau].append((equip, 1.0))
